...
--changeset tfc:041-resource-reachability splitStatements:false

ALTER TABLE scenario_resource
ADD COLUMN is_infinite BOOLEAN NOT NULL DEFAULT FALSE;

UPDATE scenario_resource
SET is_infinite=TRUE
WHERE scenario_id = 'vanilla_tfc';
...
BEGIN
    ...
    SELECT
        resource_id,
        NULL
    FROM unnest(p_initial_resources) AS resource_id;
...
CREATE FUNCTION reachable_resources(p_scenario_id TEXT)
RETURNS TABLE (
    resource_id TEXT,
    resource_name TEXT,
    reached_by_process_id TEXT
...
CREATE FUNCTION infinite_reachable_resources(p_scenario_id TEXT)
RETURNS TABLE (
    resource_id TEXT,
    resource_name TEXT,
    reached_by_process_id TEXT
