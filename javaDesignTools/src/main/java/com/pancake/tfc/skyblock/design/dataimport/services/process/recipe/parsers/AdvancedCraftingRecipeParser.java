...
public class AdvancedCraftingRecipeParser implements RecipeParser {
    ...
    public List<ProcessType> getSupportedRecipeTypes() {
        return List.of(ProcessType.ADVANCED_SHAPED_CRAFTING, ProcessType.ADVANCED_SHAPELESS_CRAFTING);
    }
    ...
    public ParsedProcess parse(
        ...
        Optional<ProcessSubtype> subType = switch (processType) {
            case ADVANCED_SHAPED_CRAFTING ->
                    Optional.of(parseShapedSubtype(recipe));
            case ADVANCED_SHAPELESS_CRAFTING ->
                    Optional.of(parseShapelessSubtype(recipe));
            default -> throw new IllegalArgumentException(
                    "Unsupported advanced crafting recipe type: " + type);
        };

        List<Set<String>> inputGroups = switch (processType) {
            case ADVANCED_SHAPED_CRAFTING -> parseShapedInputs(recipe, gameData);
            case ADVANCED_SHAPELESS_CRAFTING -> parseShapelessInputs(recipe, gameData);
            default -> throw new IllegalArgumentException(
                    "Unsupported advanced crafting recipe type: " + type);
        };
    ...
    private List<Set<String>> parseIngredientGroups(
        ...
        String type = ingredient.path("type").asString();
        ...
        if ("tfc:and".equals(type)) {
            ...
            for (JsonNode child : ingredient.get("children")) {
                groups.addAll(
                        parseIngredientGroups(child, gameData)
                );
            }
        ...
        if ("tfc:not_rotten".equals(type)) {
            return List.of();
        }

        if ("tfc:fluid_content".equals(type)) {
        ...
        if ("neoforge:difference".equals(type)) {
            Set<String> resources = new HashSet<>();

            JsonNode base = ingredient.get("base");
            JsonNode subtracted = ingredient.get("subtracted");

            Set<String> baseResources = parseIngredientGroups(base, gameData)
                    .getFirst();

            Set<String> subtractedResources = parseIngredientGroups(subtracted, gameData)
                    .getFirst();
        ...
        throw new IllegalArgumentException(
                "Unsupported advanced crafting ingredient: "
                        + ingredient
        );
    }
    ...
    private Set<String> parseResult(JsonNode recipe) {
        ...
        if (!result.has("id")) {
            throw new IllegalArgumentException(
                    "Unsupported advanced crafting result: " + result
            );
        }

        return Set.of(result.get("id").asString());
    }
    ...
    private ProcessSubtype parseShapedSubtype(JsonNode recipe) {
        ...
        throw new IllegalArgumentException(
                "Unexpected advanced crafting grid size: "
                        + rows + "x" + columns
                        + " in recipe: " + recipe
        );
    }
    ...
    private ProcessSubtype parseShapelessSubtype(JsonNode recipe) {
        ...
        throw new IllegalArgumentException(
                "Too many ingredients for advanced shapeless crafting: "
                        + ingredientCount
                        + " in recipe: " + recipe
        );
    }
